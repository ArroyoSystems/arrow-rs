...
impl ReaderBuilder {
    ...
    pub fn build_decoder(self) -> Result<Decoder, ArrowError> {
        let (data_type, nullable, metadata) = match self.is_field {
            false => (DataType::Struct(self.schema.fields.clone()), false, HashMap::new()),
            true => {
                let field = &self.schema.fields[0];
                (field.data_type().clone(), field.is_nullable(), field.metadata().clone())
            }
        };

        let decoder = make_decoder(data_type, &metadata, self.coerce_primitive, self.strict_mode, nullable)?;
...
mod tests {
    ...
    use arrow_array::cast::AsArray;
    use arrow_array::types::Int32Type;
    use arrow_array::{make_array, Array, BooleanArray, ListArray, StringArray, StructArray, Float64Array};
    ...
    fn test_deserialize_raw_json() {
        ...
        meta.insert("ARROW:extension:name".to_string(), "arroyo.json".to_string());
        let schema = Arc::new(Schema::new(vec![
    ...
    fn test_deserialize_nullable_raw_json() {
        ...
        let a = batches[0].column(0).as_primitive::<Int64Type>().value(0);
        let b = batches[0].columns().get(1).unwrap().as_any().downcast_ref::<StringArray>()
            .unwrap()
            .nulls()
            .unwrap()
            .inner()
